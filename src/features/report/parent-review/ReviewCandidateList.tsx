import type { ReviewCandidate } from '../../../mocks/seed'
...
function CandidateCard({ candidate }: { candidate: ReviewCandidate }) {
  return (
    <article className="rounded-[10px] border border-black/10 bg-white p-5">
      <p className="mb-4 text-[14px] font-semibold text-black">{candidate.label}</p>
      <div className="grid items-center gap-6 md:grid-cols-[1.4fr_1fr]">
        <div className="space-y-3">
          ...
          <div>
            ...
            <p className="mt-1 text-[12px] leading-relaxed text-black">{candidate.aiAnalysis}</p>
          </div>
          <div>
            <p className="text-[12px] text-black/50">낮은 확신도 사유</p>
            <p className="mt-1 text-[12px] leading-relaxed text-black">
              {candidate.lowConfidenceReason}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4 rounded-[8px] border border-black/10 bg-[#f3f4f6] p-4">
          <div>
            ...
            <p className="mt-1 text-[12px] text-black">{candidate.before}</p>
          </div>
          <div>
            <p className="text-[12px] text-black/50">수정 후 (AI 제안)</p>
            <p className="mt-1 text-[12px] text-black">{candidate.after}</p>
          </div>
        </div>
      </div>
    </article>
...
export function ReviewCandidateList({ candidates }: { candidates: ReviewCandidate[] }) {
  return (
